...
LRESULT CALLBACK DialogCamera(HWND hDlg,UINT uMsg,WPARAM wParam,LPARAM lParam)
{
	switch(uMsg)
	{
	case WM_INITDIALOG:
		{
			char buffer[32]; 
			sprintf(buffer, "%f", Camera3D->Position.x);		
			SetDlgItemText(hDlg, IDC_EDIT1, buffer);
			sprintf(buffer, "%f", Camera3D->Position.y);		
			SetDlgItemText(hDlg, IDC_EDIT2, buffer);
			sprintf(buffer, "%f", Camera3D->Position.z);		
			SetDlgItemText(hDlg, IDC_EDIT3, buffer);
			sprintf(buffer, "%f", Camera3D->LookAtPoint.x);		
			SetDlgItemText(hDlg, IDC_EDIT4, buffer);
			sprintf(buffer, "%f", Camera3D->LookAtPoint.y);		
			SetDlgItemText(hDlg, IDC_EDIT5, buffer);
			sprintf(buffer, "%f", Camera3D->LookAtPoint.z);		
	...
	case WM_COMMAND:
		...
		{
			char buffer[32];			
			GetDlgItemText(hDlg, IDC_EDIT1, buffer, 32);
			Camera3D->Position.x = atof(buffer);	
			GetDlgItemText(hDlg, IDC_EDIT2, buffer, 32);
			Camera3D->Position.y = atof(buffer);	
			GetDlgItemText(hDlg, IDC_EDIT3, buffer, 32);
			Camera3D->Position.z = atof(buffer);
			GetDlgItemText(hDlg, IDC_EDIT4, buffer, 32);
			Camera3D->LookAtPoint.x = atof(buffer);	
			GetDlgItemText(hDlg, IDC_EDIT5, buffer, 32);
			Camera3D->LookAtPoint.y = atof(buffer);	
			GetDlgItemText(hDlg, IDC_EDIT6, buffer, 32);
			Camera3D->LookAtPoint.z = atof(buffer);	
	...
	case WM_COMMAND:
		{
			...
			case ID_MENU_ABOUT:
				{
					char buffa[160];
					LoadString(hInstance,IDS_STRING1,buffa,160);
					MessageBox(NULL, buffa, "About",0); 

				} break;

			case ID_MENU_CAMERA:
				{
					DialogBox(hInstance, (LPCTSTR)IDD_DIALOG_CAMERA, hWnd, (DLGPROC)DialogCamera);
...
int WINAPI WinMain(HINSTANCE hInst,HINSTANCE hPrevInst,LPSTR,int)
{
	...
	Accelerators=LoadAccelerators(hInst, MAKEINTRESOURCE(IDR_ACCELERATOR1));
