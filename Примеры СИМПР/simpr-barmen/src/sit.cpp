...
class Situation
{
	public:
		...
		bool moveB() {mpB->mdXPos+=mpB->mnDir; mpB->mnM=1;if(!mpB->k) mpB->mnBN = 2 - mpB->mnDir; else mpB->k--; return 1;}
		bool give() {for(int i=0;i<mP.size();i++) if(mP[i]->mnSit) {mP[i]->h=true; mBS.mfSt=true;} mpB->mnM=1;mpB->mnBN=2; mpB->k=10; return 1;}
	protected:
		Storage<Man> mP;
		Barmen* mpB;
		BStorage mBS;
